...
sns.set(style="ticks")
# 修改以下两行：
# matplotlib.rc("text", usetex=True)  # 改为 False
# plt.rcParams['text.usetex'] = True   # 改为 False
...
#
#     # 生成新的等间距网格
#     x_new = np.linspace(x_orig.min(), x_orig.max(), 100)  # 自定义点数
#     y_new = np.linspace(y_orig.min(), y_orig.max(), 100)
#     X_new, Y_new = np.meshgrid(x_new, y_new)
#
#     # 插值 u, v 到新网格
#     points = np.array([(x, y) for x in x_orig for y in y_orig])
#     u_interp = griddata(points, u.flatten(), (X_new, Y_new), method='linear')
#     v_interp = griddata(points, v.flatten(), (X_new, Y_new), method='linear')
#
#     # 绘制流线图
#     stream_set = ax.streamplot(x_new, y_new, u_interp, v_interp, color="C0", linewidth=0.5, density=4, arrowsize=0)
#     stream_set.lines.set_alpha(0.3)
...
def plot_streamlines(ax, u, v, data):
    ...
    if isinstance(x_orig, torch.Tensor):
        ...
        y_orig = y_orig.detach().cpu().numpy()
    ...
    u_interp = griddata(points, u.flatten(), (X_new, Y_new), method='linear')
    v_interp = griddata(points, v.flatten(), (X_new, Y_new), method='linear')
    u_interp = np.asarray(u_interp)  # 强制转换
    v_interp = np.asarray(v_interp)

    # 绘制流线
    stream_set = ax.streamplot(x_new, y_new, u_interp, v_interp,
                               color="C0", linewidth=0.5, density=4, arrowsize=0)
    stream_set.lines.set_alpha(0.3)


def grid_reshape(y):
    # 如果输入是 Tensor，转换为 NumPy
    if isinstance(y, torch.Tensor):
        y = y.detach().cpu().numpy()
    # 确保输出为 NumPy 并重塑形状
    return np.asarray(y).reshape(*CylinderFlowData.adjusted_grid)
...
def main(reload: bool = False):
    ...
    plot_latent_states(t_eval, ys, data, "valid_z", "latent_state_preds_10_gru.pdf")
